...
function fgLabel(score: number) {
  if (score <= 20) return { label: "Peur extreme", color: "var(--red)" };
  if (score <= 40) return { label: "Peur", color: "#f97316" };
  if (score <= 60) return { label: "Neutre", color: "var(--muted)" };
  if (score <= 80) return { label: "Avidite", color: "var(--green)" };
  return { label: "Avidite extreme", color: "var(--green)" };
}
...
export default function RiskCard({ data }: { data: RiskData | null }) {
  if (!data) {
    return (
      <div className="rounded-xl p-5 border" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
        <h2 className="text-lg font-bold mb-3" style={{ color: "var(--accent)" }}>Liberty Risk</h2>
  ...
  const fg = fgLabel(data.fearGreed);

  return (
    <div className="rounded-xl p-5 border" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold" style={{ color: "var(--accent)" }}>Liberty Risk</h2>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
          <div className="text-xs" style={{ color: "var(--muted)" }}>Fear & Greed</div>
          <div className="font-medium" style={{ color: fg.color }}>{data.fearGreed} — {fg.label}</div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="font-medium">${data.priceBtc.toLocaleString("fr-FR")}</div>
        </div>
        <div>
          <div className="text-xs" style={{ color: "var(--muted)" }}>ETH</div>
          <div className="font-medium">${data.priceEth.toLocaleString("fr-FR")}</div>
        </div>
      </div>

      {/* Summary */}
      {data.marketSummary && (
        <div className="mt-4 text-xs italic" style={{ color: "var(--muted)" }}>
          {data.marketSummary}
        </div>
      )}
